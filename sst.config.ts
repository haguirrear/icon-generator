...
export default $config({
  app(input) {
    return {
      ...
      providers: {
        aws: {
          profile:
            input.stage === "production" ? "icon-production" : "icon-dev",
        },
        cloudflare: "5.47.0",
      },
    };
  ...
  async run() {
    ...
    const remixApp = new sst.aws.Remix("RemixApp", {
      domain: {
        // name: $app.stage == "production" ? "therevol.co" : "dev.therevol.co",
        name: "therevol.co",
        redirects: ["www.therevol.co"],
      ...
      environment: {
        NODE_ENV: ["dev", "production"].includes($app.stage)
          ? "production"
          : "dev",
        IS_DEV: $dev.toString(),
        SITE_URL: $dev
          ? "http://localhost:5173"
          : siteUrlMap.get($app.stage) || "",
